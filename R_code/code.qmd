...
$$Y_{ij} = \beta1 + \beta2 \text{ Post}_{ij} + \beta3 \text{ Length}_i + \beta4 \text{ Post}_{ij} \times \text{Treatment}_{i} + b_{1i} + b_{2i} \text{ Post}_{ij} + \epsilon_{ij}$$
```{r}
summary(mod1.slope1)
...
#spline model
#dropped drug
mod1.slope.knot <- lme(score ~ month + length + knot + month:treatment + knot:treatment, 
                       random = ~ month + knot| id, data = btb.long, na.action = na.omit)
