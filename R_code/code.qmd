...
```{r}
#spaghetti plot with Marginal Mean Response Over Time
...
```{r}
getVarCov(mod1.slope1) # G matrix

mod1.slope1$varFix # covariance of fixed effects (beta) estimates
```

```{r}
# 95% CI for TAU group
-6.128568 + c(-1, 1)*qnorm(0.975)*1.281959

# 95% CI for BtheB group 
-6.128568-3.063345
SE <- sqrt(1.64341914 + 2.77502525 + 2*0.09859223)
-9.191913 + c(-1, 1)*1.96*SE
...
21.776532 + c(-1,1)*qnorm(0.975)*9.367765 # random intp

-3.063345 + c(-1,1)*qnorm(0.975)*6.648816 # random slope
```
